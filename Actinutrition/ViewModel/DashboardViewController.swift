...
//  Actinutrition
//
//  Created by Elaine Reyes on 1/3/18.
...
class DashboardViewController: UIViewController, WKUIDelegate, WKNavigationDelegate, UIScrollViewDelegate
{
    ...
    @IBOutlet var topSpaceStepCounter: NSLayoutConstraint!
    ...
    func addWKWebViewToView()
    {
        ...
        if UIScreen.main.nativeBounds.height == 2436
        {
            ...
            {
                topSpaceStepCounter.constant = UIApplication.shared.statusBarFrame.size.height + (self.navigationController?.navigationBar.frame.size.height)!
            }
        }
        ...
        mainWebview.frame = CGRect(x: 0, y: UIApplication.shared.statusBarFrame.size.height + (self.navigationController?.navigationBar.frame.size.height)! + yValueToAdd, width: tempView.frame.size.width, height: tempView.frame.size.height - minusiPhoneXHeight - statusBarHeight - toolbarHeight)
    ...
    func loadURL()
    {
        ...
        userAgent = NSString(format: "%@/%@ %@ Mobile %@", "Actinutrition", Bundle.main.object(forInfoDictionaryKey: "CFBundleVersion") as! NSString, deviceName(), userAgent!) as String
